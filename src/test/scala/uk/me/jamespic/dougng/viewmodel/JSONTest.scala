...
class JSONTest extends FunSpec with Matchers with RegisteringMixin
  ...
  describe("A JSON object") {
    ...
    it("should serialize Maps to Json objects") {
      ...
      instance.dumps(Map("Hello" -> 1)) should equal("{\"Hello\": 1}")
      instance.dumps(Map(true -> BigDecimal(1))) should equal("{true: 1}")
    }
    ...
    it("should serialize ODocuments") {
      ...
      doc.save()
      instance.dumps(doc) should equal("""{"@rid":"#13:0","@version":0,"@class":"TestDBClass","name":"MyName","value":1,"data":["Hello","World"],"child":{"@rid":"#3:0","@version":0,"data":1},"@fieldTypes":"value=l"}""")
    }
    ...
    it("should serialize registered POJOs") {
      ...
      instance.dumps(timegraph) should equal("""{"@rid":"#12:0","@version":0,"@class":"TimeGraph","granularity":20000,"maxDatasets":[],"datasets":[],"hiddenRows":["Row1"],"rowColours":{"Row1":"#f00"},"name":"MyTimeGraph"}""")
    }
